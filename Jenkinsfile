      ...
      steps {
        sh '''
          ...
          if ! command -v node >/dev/null 2>&1 || ! node --version | grep -q "v18"; then
            ...
            # Set ARCH with proper default value
            ARCH=$(uname -m)
            case $ARCH in
              x86_64) ARCH="x64" ;;
              aarch64) ARCH="arm64" ;;
              armv7l) ARCH="armv7l" ;;
              *) ARCH="x64" ;;  # Default to x64 if unknown architecture
            esac
            ...
            { echo "Failed to download Node.js binary for architecture $ARCH"; exit 1; }
...
EOF2
    ...
    stage('Web Build') {
      ...
      post {
        always { 
          archiveArtifacts artifacts: '${WEB_OUT}/**', allowEmptyArchive: true 
        }
      }
    }
